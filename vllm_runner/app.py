...
with gr.Blocks(title="Tmux Manager with GPU Monitor") as demo:
    with gr.Row():
        with gr.Column(scale=2):
            max_length = gr.Number(label="Max Model Length", value=16_000)
            tp = gr.Dropdown(label="Tensor Parallel Size", choices=[1, 2, 4, 8], value=4)
            model_selection = gr.Dropdown(
                label="Model Selection", choices=get_model_selections(), value="32B"
            )
...
# Manage vllm process
# with gr.Blocks(title="vLLM Process Manager") as demo:
    from vllm_runner.scan_vllm_process import scan_vllm_process
    
    def format_process_name(process_dict):
        # example process_dict [{'pid': 1066, 'port': '2800', 'model_name': 'Qwen2.5-72B-Instruct-AWQ'}, {'pid': 3335, 'port': '2801', 'model_name': 'Qwen2.5-32B-Instruct-AWQ'}]
        processes = {}
    ...
    def refresh_process_list():
        processes = format_process_name(scan_vllm_process())
        return gr.Dropdown(choices=list(processes.keys()))
    ...
    with gr.Row():
        ...
        refresh_btn = gr.Button("Refresh Process List")
